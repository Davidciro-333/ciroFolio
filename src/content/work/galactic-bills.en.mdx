...
<Pipeline
  branchLabel="else"
  caption="Two checks decide everything: one before the model runs, one after. Neither of them existed in the first version."
  steps={[
    { n: '01', title: 'Ingest', detail: 'A photo or PDF arrives through the Telegram bot, or is dropped into the shared Drive folder.' },
    { n: '02', title: 'Pick up', detail: 'The file is stored and the pipeline detects it in under a minute.' },
    { n: '03', title: 'Classify', detail: 'Is this actually an invoice?', branch: 'it is not — rejected here, before it can pollute the books.' },
    { n: '04', title: 'Extract', detail: 'Gemini reads the fiscal fields against a fixed schema, at temperature 0, with a strict JSON response type.' },
    { n: '05', title: 'Validate', detail: 'Taxable base plus VAT has to equal the total.', branch: 'the numbers do not add up — goes to a human review queue over Telegram, never to the spreadsheet.' },
    { n: '06', title: 'File', detail: 'The row is written and deduplicated by a hash of the file itself.' },
    { n: '07', title: 'Confirm', detail: 'The client hears back on the same channel they used.' },
  ]}
/>
